...
mPeakClustering :: mPeakClustering(iforest &iForestObject,vector<vector<double>> &massMatrix, const double & Dc):iForestObject(iForestObject),_massMatrix(massMatrix), _Dc(Dc){
...
void mPeakClustering :: computeLocalMass(){
...
int mPeakClustering :: numberOfClusterCenters(){
...
void mPeakClustering :: clusterAssignment(){
...
vector<double> mPeakClustering :: getDeltaVector() const{
    return this->_delta;
}
vector<int> mPeakClustering :: getNeighbourVector() const{
    return this->_nneigh;
}

vector<pair<double,int>> mPeakClustering :: getOrdRho(){
    return this->_ordrho;
}
